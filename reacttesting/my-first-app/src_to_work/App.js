...
class SelectedRepoList extends React.Component {
  // showPullRequest(i) {
        ...
        <tbody>
          ...
          {  this.props.data.map( ( repo, index ) => (
            <tr key={index}>
              ...
              <td>{repo.forks}</td>
              <td><button onClick={this.props.myFunction.bind(this,repo.pulls_url.replace('{/number}', "?state=all"))}>Show</button></td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  render() {
    ...
    for (let i = this.state.page*10; i < (this.state.page+1)*10; i++) {
      if(this.props.repositories[i]) {
        tmp.push(this.props.repositories[i])
      }
    }
    console.log(tmp);
    return (
      <div>
        <SelectedRepoList data={tmp} myFunction={this.showPullRequest}/>
        <table >
          <tbody>
            <tr>
              <td>page No.</td>
              {[...Array(Math.floor((this.props.repositories.length+10) / 10))].map( (x, i) => (
                <td key={i}><button onClick={() => {this.setState({page: i+1});  console.log(this.state.page);}}>{i+1}</button></td>
              ))}
            </tr>
          </tbody>
        </table>
